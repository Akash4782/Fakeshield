...
const FAQItem: React.FC<{ question: string; answer: string; icon: React.ReactNode }> = ({ question, answer, icon }) => {
  ...
  return (
    <div className={`p-8 rounded-[2.5rem] border transition-all duration-500 ${isOpen ? 'border-[#00E5CC]/30' : ''}`} style={{ background: 'var(--panel-bg)', borderColor: isOpen ? 'rgba(0,229,204,0.3)' : 'var(--panel-border)' }}>
      <button 
      ...
      >
        <div className="flex items-center gap-6">
          ...
          <h3 className="text-lg font-bold leading-tight" style={{ color: 'var(--text-heading)' }}>{question}</h3>
        </div>
      ...
      <div className={`overflow-hidden transition-all duration-500 ${isOpen ? 'max-h-96 opacity-100 mt-8' : 'max-h-0 opacity-0'}`}>
        <p className="font-medium leading-relaxed pl-[4.5rem]" style={{ color: 'var(--text-secondary)' }}>
          {answer}
        </p>
      </div>
    </div>
